...
def download_results(analysis_type):# تنزيل النتائج
    ...
    if cheating_report: # اذا في ريبورت غش
        ...
        for pair, similarity in cheating_report.items(): # for pair لطالبين 
            ...
            essay_submissions = session.get('essay_submissions', {})
            student_a_content = essay_submissions.get(student_a, 'Not found')
            flash(f"Student A content: {student_a_content}", 'info')
